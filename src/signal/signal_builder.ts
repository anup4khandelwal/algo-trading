...
export interface SignalBuilderConfig {
  ...
  riskPerTrade: number;
  minCapitalDeployPct?: number; // optional floor: min notional per trade as % of equity
  minAdv20: number;
  ...
  buildSignals(results: ScreenerResult[], equity: number): Signal[] {
    ...
    return sorted
      .map((r) => {
        ...
        const qtyByRisk = Math.floor(capitalAtRisk / perShareRisk);
        const minDeployPct = Math.max(0, this.cfg.minCapitalDeployPct ?? 0);
        const qtyByMinDeploy =
          minDeployPct > 0 ? Math.ceil((equity * minDeployPct) / Math.max(entry, 0.01)) : 0;
        const qty = Math.max(qtyByRisk, qtyByMinDeploy);
